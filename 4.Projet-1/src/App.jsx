    ...
    <div className="h-screen">
      <div className="max-w-4xl mx-auto pt-20 px-6">
        ...
        <form onSubmit={handleSubmit} className="mb-10">
          ...
          />
          {showValidation && <p className="text-red-400">Ajouter d'abord du contenu à votre tâche</p>}
          <button className="my-4 py-2 px-2 bg-slate-50 rounded min-w-[115px]">Ajouter</button>
        </form>
        <ul>
          {todoList.length === 0 && <li className="text-slate-50 text-md">Pas d'items à afficher...</li>}
          {todoList.length > 0 &&
            todoList.map((item) => <ListItem key={item.id} itemData={item} deleteTodo={deleteTodo} />)}
        </ul>
      </div>
    </div>
  );
